...
func _show_pallete_items(pallete_items: Array[AssetResource]) -> void:
	...
	for index in range(pallete_items.size()):
		...
		item_instance.on_add_asset_click.connect(func():
			AssetPickerDialog.open(func(asset: AssetResource):
				_configure_shortcut_key(asset, index)
			)
		)
		item_instance.on_clear_asset_click.connect(func():
			presenter.remove_slot(index)
		)
