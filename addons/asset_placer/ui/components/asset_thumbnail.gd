...
func preview_resource(resource: AssetResource):
	if Engine.is_editor_hint() and resource and resource.get_resource():
		last_time_modified = FileAccess.get_modified_time(resource.get_path())
		previewer = EditorInterface.get_resource_previewer()
		previewer.queue_edited_resource_preview(resource.get_resource(), self, "_on_preview_generated", null)

func _process(_delta):
	if Engine.is_editor_hint() and resource and resource.get_resource():
		var new_time_modified = FileAccess.get_modified_time(resource.get_path())
