...
Pre-release alpha for the upcoming **1.7.0** line. This release adds **Persistent Placement Options** across editor sessions 
### ✨ New Features
...
A new streamlined dialog for managing asset collections, accessible from the Collections tab or by right-clicking an asset.

![](./docs/assets/cm_preview.png)

**Features:**

- **Batch Selection** - Select multiple assets using Shift+Click (range) or Ctrl/Cmd+Click (multi-select) to assign or remove collections from multiple assets simultaneously
- **Visual Feedback** - See which collections are fully assigned, partially assigned, or available for each selection
- **Dialog Size Persistence** - The dialog remembers its size across sessions

#### Automatic Collections Grouping

A new option to automatically group placed assets under nodes named after their **primary collection**, keeping your scene tree organized without manual effort.

When enabled in the Asset Placement Options, your scene tree will be organized like this:
...
Each asset can now have an explicitly set **primary collection** that determines grouping behavior.

![](./docs/assets/manage_collections.png)

- **Primary Collection Indicator** - The Collection Management dialog shows a star icon (tinted with the collection's color) to indicate and toggle the primary collection
- **Explicit Assignment** - Primary collection must be explicitly set by the user; it is not automatically assigned
- **Easy Toggle** - Click the star on an already-primary collection to remove primary status
- **Batch Assignment** - When multiple assets are selected, the star button is always visible for easier batch primary assignment

#### Folder Rules

Configurable rules for asset folders that automate actions when syncing.
...
Rules are configured per-folder and can be combined. Click the "Rules" button on any folder to expand the rules panel.
